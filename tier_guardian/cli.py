...
def main():
    ...
    parser.add_argument(
        "text", nargs="?", help="要审核的文本内容"
    )
    parser.add_argument(
        "--file", "-f", help="从文件读取文本（每行一条）"
    )
    parser.add_argument(
        "--config", "-c", default=None, help="配置文件路径 (JSON/YAML)"
    )
    parser.add_argument(
        "--scene", "-s", default="comment", help="场景类型 (默认: comment)"
    )
    parser.add_argument(
        "--locale", "-l", default="zh-CN", help="语言区域 (默认: zh-CN)"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="详细输出"
    )
    parser.add_argument(
        "--json-output", action="store_true", help="以 JSON 格式输出"
    )
...
def _process_single(orch: Orchestrator, text: str, args) -> None:
    ctx = orch.process(text, scene=args.scene, locale=args.locale)
...
def _process_file(orch: Orchestrator, args) -> None:
    ...
    for line in lines:
        ...
        if not line:
            continue
        ctx = orch.process(line, scene=args.scene, locale=args.locale)
        results.append(ctx)
    ...
    if args.json_output:
        ...
        for i, ctx in enumerate(results):
            print(f"[{i + 1}] {ctx.final_decision.value if ctx.final_decision else 'UNKNOWN'}: {ctx.text[:80]}")
...
def _output_result(ctx, args) -> None:
    if args.json_output:
        ...
        if ctx.nodes.intent:
            print(f"  意图: {ctx.nodes.intent.intent.value} (置信度: {ctx.nodes.intent.confidence:.2f})")
        if ctx.nodes.judge:
            v = ctx.nodes.judge.violation
            print(f"  违规判定: {v.is_violation} (类型: {v.type}, 严重度: {v.severity.value if v.severity else 'N/A'}, 置信度: {v.confidence:.2f})")
